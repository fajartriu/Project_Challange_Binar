...
//    List<OrderDTO> getAllUserOrder(@Param("userId") UUID userId);
    ...
    @Query("select new project.Challenge_4BinarFood.model.order.OrderDTO(o.order_time, u.username, p.productName, m.merchantName, o.destination_address, od.quantity, p.price, od.total_price, o.completed) from Users u JOIN u.order o JOIN o.orderDetail od JOIN od.product p JOIN p.merchant m where u.id=:userId")
    List<OrderDTO> getAllUserOrder(@Param("userId") UUID userId);

    @Query("select new project.Challenge_4BinarFood.model.order.OrderDTO(o.order_time, u.username, p.productName, m.merchantName, o.destination_address, od.quantity, p.price, od.total_price, o.completed) from Users u JOIN u.order o JOIN o.orderDetail od JOIN od.product p JOIN p.merchant m")
