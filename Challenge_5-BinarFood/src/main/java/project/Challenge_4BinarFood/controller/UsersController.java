...
public class UsersController {
    ...
    public WebResponse<UsersResponse> getUserById(@PathVariable UUID userId){
        try {
            UsersResponse userById = usersService.getUserById(userId);
            return WebResponse.<UsersResponse>builder().countRecord(1).data(userById).messages("Success").build();
        }catch (Exception e){
            return WebResponse.<UsersResponse>builder().data(null).messages(e.getMessage()).build();
        }
    }
